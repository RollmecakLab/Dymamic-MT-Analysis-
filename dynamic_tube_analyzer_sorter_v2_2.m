...
SlowGrowers=find((CurrentChamber(:,1)<0.1 & CurrentChamber(:,1)>0 ));
...
for(i=1:NumberOfTubes)
    ...
    currentTubeData = CurrentChamber(currentTubeRows(1):currentTubeRows(end), 1:5); %gets the data on the current tube
      ...
      %- now check for Age At Cat...adn back calculate as neccessary
      %atAge(1,2)=-12345;C
        for ( N=1:numel(CatEvents))
            
            if( abs(currentTubeData(CatEvents(N),2)) >  currentTubeData(CatEvents(N)-1,2))
%                % if it i sbigger back calculate the age...
        ...
        %- next part added May 1 2016 ;
        ShrinkEvents=find(currentTubeData(:,1)<0);
        for(N=1:numel(ShrinkEvents))
            CatRates(end+1,2)=abs(currentTubeData(ShrinkEvents(N),1));
        end
